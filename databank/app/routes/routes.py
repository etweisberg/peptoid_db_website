...
def get_home(peptoids):
    ...
    return [peptoid_codes, peptoid_urls, peptoid_titles, peptoid_sequences, images, data, publications]
...
def home():
    properties = get_home(
        [p for p in Peptoid.query.order_by(Peptoid.release.desc()).all()])
...
def experiment(var):
    properties = get_home([p for p in Peptoid.query.order_by(
        Peptoid.release.desc()).filter_by(experiment=var).all()])
...
def doi(var):
    ...
    peptoids = Peptoid.query.order_by(Peptoid.release.desc()).filter(
        (Peptoid.struct_doi == var) | (Peptoid.pub_doi == var)).all()
...
def topology(var):
    properties = get_home([p for p in Peptoid.query.order_by(
        Peptoid.release.desc()).filter_by(topology=var).all()])
